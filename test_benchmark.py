...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/COCO/DsiacPlus.data', help='*.data path')
    ...
    parser.add_argument('--task', default='study', help="'test', 'study', 'benchmark'")
    ...
    if opt.task == 'test':  # task = 'test', 'study', 'benchmark'
        ...
        for i in x:
            print("Threshold:",i)
            # Run the Test on Desired mAP Score
            line = test(opt.cfg, opt.data, opt.numCount, opt.weights, opt.batch_size, opt.img_size, opt.conf_thres, i, opt.save_json)[0]
            with open("mAP_Study.txt", 'a') as f:
                new_line = str(i) + " " + str(line[0]) + " " + str(line[1]) + " " + str(line[2]) + " " + str(line[3]) + " \n"
                f.write(new_line)
            r = line[2]
            #print(r)
            #print(type(r))
            average[index] = r
            index += 1
        mAP_score = np.average(average)
        print("mAP0.5:0:95", mAP_score)